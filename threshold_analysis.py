"""
For each anchor, filters words at threshold <= N missing sources (default N=2,
meaning missing from FEWER than 3 datasets = "high frequency" words).

For each source, reports what % of those high-frequency words are absent in that source.
Also writes filtered CSVs: threshold_N_anchor_{NAME}.csv
"""
...
for consol_path in sorted(glob.glob(os.path.join(DATA_DIR, "consolidated_anchor_*.csv"))):
    anchor_name = (
        os.path.basename(consol_path)
        .removeprefix("consolidated_anchor_")
        .removesuffix(".csv")
    )
    ...
    # Filter to high-frequency words (missing from <= THRESHOLD checked sources)
    hf_rows = [r for r in rows if missing_count(r) <= THRESHOLD]
    n_hf = len(hf_rows)

    # For each source (all, not just checked), compute absence rate in high-freq set
    source_absent = {}
    ...
    # Write filtered CSV
    out_path = os.path.join(DATA_DIR, f"threshold_{THRESHOLD}_anchor_{anchor_name}.csv")
...
report_path = os.path.join(DATA_DIR, f"threshold_analysis_N{THRESHOLD}.md")
...
for anchor_name, data in all_anchor_results.items():
    ...
    # Sort by absence count descending (worst culprits first)
