...
What this means for you: if a word ranks high across `BCCWJ`, `RSPEER_rank`, `CEJC_rank`, and `ANIME_JDRAMA`, it is genuinely common across formal written, multi-source, spoken, and media Japanese. If it only ranks high in a few niche columns, its utility depends on your specific use case.
...
**`BCCWJ`** is the authoritative reference for formal written Japanese. NINJAL built it from 104 million words spanning books, magazines, newspapers, blogs, legal documents, textbooks, and more (1976–2006), deliberately balanced across text types to avoid single-domain bias. Only ~16k entries in this CSV, but within that range the ranks are the most authoritative signal of written Japanese importance. A -1 reliably indicates a word is colloquial, very modern, or spoken-only.
...
`NETFLIX` and `DAVE_DOEBRICK` both use surface-form tokenization but with different tools, so they can diverge on how compound words and verb forms are handled. `DD2_MORPHMAN_NETFLIX` uses UniDic lemmas, which means some inflected forms are merged — closer to what lemmatized sources like `BCCWJ` and `CEJC_rank` produce. Because proper names are filtered out in `DD2_MORPHMAN_NETFLIX`, it has fewer entries (~105k) than `DAVE_DOEBRICK` (~124k).
