"""
Generates DATA.csv from the JPDB Yomitan frequency dictionary (Shoui/MarvNC collection).
Source: term_meta_bank_1.json — [word, "freq", {"value": rank, "displayValue": "rank㋕"}]
Excludes entries marked with ❌ (not in JPDB corpus).
...
INPUT_FILE = os.path.join(
    ...
    "../../SHOUI/[Freq] JPDB (Recommended)/term_meta_bank_1.json",
)
    ...
    for entry in data:
        ...
        try:
            if isinstance(freq_data, dict):
                display = freq_data.get("displayValue", "")
                if "❌" in str(display):
                    continue  # skip entries not in JPDB corpus
                rank = float(freq_data.get("value", float("inf")))
            else:
