"""
Runs coverage analysis on every {ANCHOR}_anchor/consolidated.csv in data/ALL/.
For each anchor, produces:
  - coverage_analysis_anchor_{NAME}.md  (per-source missing %, rank-band breakdown, distribution)

Updated EXCLUDE set (vs experiments0): adds H_FREQ, NAROU, VN_FREQ per §12 recommendation.
Runs on all *_anchor/ directories — picks up new anchors (BCCWJ, CC100, RSPEER, WIKIPEDIA_V2)
automatically.
"""
...
for anchor_dir in sorted(glob.glob(os.path.join(DATA_DIR, "*_anchor"))):
    ...
    neg_path = os.path.join(BASE, f"has_negative_rank_anchor_{anchor_name}.csv")
    rare_path = os.path.join(BASE, f"has_rare_category_anchor_{anchor_name}.csv")
    ...
    with open(categ_path, newline="", encoding="utf-8") as f:
        ...
        cat_check = [c for c in cat_header if c not in ("word", "hiragana", "katakana") and c not in anchor_family and c not in EXCLUDE]
