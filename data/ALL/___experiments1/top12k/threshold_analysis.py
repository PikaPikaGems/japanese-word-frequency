"""
Threshold analysis on top-12k slices of each anchor.
...
for anchor_dir in sorted(glob.glob(os.path.join(DATA_DIR, "*_anchor"))):
    ...
    source_absent = {col: sum(1 for r in hf_rows if r[col] == "-1") for col in all_source_cols}
...
lines = [
    f"# Threshold Analysis (Top 12k) — High-Frequency Words (Missing from ≤{THRESHOLD} Sources)",
    ...
    f"**Word list:** Top 12,000 words per anchor (all anchors capped equally for comparability).",
    f"\n---",
]
...
for anchor_name, data in all_anchor_results.items():
    ...
    for col in sorted(all_source_cols, key=lambda c: -source_absent[c]):
        ...
        lines.append(f"{col:<35} {absent:>7}  {pct:>5.1f}%  {bar(absent, n_hf)}{excl_tag}")
