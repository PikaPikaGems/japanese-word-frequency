...
The source TSV (`2_cejc_frequencylist_suw_token.tsv`) contains **29,534 total entries** across **27,988 unique word spellings** and **719 distinct frequency levels**.

## Standard Competition Ranking (1224)

CEJC uses **standard competition ranking (1224)**, not dense ranking. Words with identical corpus frequency share the same rank, and the next rank skips accordingly. This creates enormous tie-groups at the low-frequency end of the spectrum:

| Rank   | Entries sharing this rank | Corpus freq (each) |
| ------ | ------------------------- | ------------------ |
| 20,704 | 8,831                     | 1                  |
| 16,266 | 4,438                     | 2                  |
| 13,446 | 2,820                     | 3                  |
...
The highest rank value is **20,704** even though there are only **719 unique frequency levels** — a gap of 19,985 caused entirely by tie-induced rank jumps. Only **430 words** have a frequency unique enough to earn their own rank. See `insights/rank_distribution.md` for the full analysis.

## Consolidated CSVs

Two derived files are generated from the full dataset:

- **`CONSOLIDATED.csv`** — 29,534 rows. One row per entry (word + POS combination), sorted by `combined_rank`. Columns: `word`, `combined_rank`, and rank columns for all 11 domains and 2 gender demographics.
- **`CONSOLIDATED_UNIQUE.csv`** — 27,988 rows. Deduplicated by word spelling, keeping the entry with the best (lowest) `combined_rank` per word. Generated by `scripts/make_consolidated_unique.py`.
...
`top_n` is optional and defaults to 12,000. Since the dataset only has 29,534 entries with a max rank of 20,704, any `top_n` above 20,704 captures the full dataset. Previously generated files with `top_n=12,000` are preserved in `csv_12k/` and `json_12k/`; current files in `csv/` and `json/` use `top_n=25,000` (i.e. the full dataset).

# Generated JSON Files

## Introduction

Three files are output by `scripts/tsv_to_json.py`, all covering the top `top_n` words by overall frequency rank. Each word key maps to an **array of entries** — one per POS — because the same surface form can appear multiple times with different grammatical categories (e.g. の as 助詞-格助詞, 助詞-準体助詞, 助詞-終助詞). All leaf values are `[rank, freq, pmw]` arrays. Each word key maps to an **array of entries** — one per POS — because the same surface form can appear multiple times with different grammatical categories (e.g. の as 助詞-格助詞, 助詞-準体助詞, 助詞-終助詞). All leaf values are `[rank, freq, pmw]` arrays.
