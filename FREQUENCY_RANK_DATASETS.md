...
Each word in this database is assigned a **frequency rank** from each dataset below. A rank of **1** means it is the single most frequent word in that corpus. A rank of **25,000** means it is the 25,000th most frequent. A **blank/missing value** means the word either did not appear in that corpus, or fell outside the top 25,000 (or top ~187k for YOUTUBE_FREQ_V3).
...
**Is a word blank in most datasets?** → It is probably rare, domain-specific, or a proper noun.

**Is a word ranked very high in entertainment datasets but blank in BCCWJ/CEJC?** → It is likely fiction/media jargon not used in daily life.

**Is a word ranked very high in CEJC but blank in BCCWJ?** → It is a common spoken word that rarely appears in formal written text.
...
|--------|---------------|
| `combined_rank` | Overall CEJC frequency across all 577 conversations and all domains |
| `small_talk_rank` | Frequency within casual small talk conversations (雑談) |
| `consultation_rank` | Frequency within counseling/advice consultations (相談) |
| `meeting_rank` | Frequency within formal meetings (会議) |
| `class_rank` | Frequency within classroom/lesson settings (授業) |
| `outdoors_rank` | Frequency within outdoor activity conversations (屋外活動) |
| `school_rank` | Frequency within school environment conversations (学校生活) |
| `transportation_rank` | Frequency within transportation contexts (交通機関) |
| `public_commercial_rank` | Frequency within public/commercial service interactions (公共・商業施設) |
| `home_rank` | Frequency within home/domestic conversations (家庭生活) |
| `indoors_rank` | Frequency within indoor non-home settings (屋内施設) |
| `workplace_rank` | Frequency within workplace conversations (職場) |
| `male_rank` | Frequency in speech produced by male speakers |
| `female_rank` | Frequency in speech produced by female speakers |
...
**Best for:** Academic spoken Japanese, professional presentations, formal speech. If a word ranks highly in CSJ but not CEJC, it is likely formal/academic vocabulary used in presentations rather than casual daily speech. The dialogue sub-corpora (TAIKA_*) are closer to natural conversation than the monologue ones (DOKWA_*).
...
## Section 6 — Dave Doebrick Compilation Variants (DD2_*)
